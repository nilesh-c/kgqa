...
class LCQuADMmlSemanticParsingSimple():
    ...
    def setUp(self):
        ...
        self.dataset_sample_file_path = self.TEST_DATA_ROOT / "lcquad.annotated.lisp.v2.deurified.simple.sample.json"
        self.dataset_train_file_path = self.TEST_DATA_ROOT / "lcquad.annotated.lisp.v2.deurified.simple.train.json"
        self.dataset_test_file_path = self.TEST_DATA_ROOT / "lcquad.annotated.lisp.v2.deurified.simple.test.json"
        ...
        if self.sample_only:
            ...
            self.train_instances = list(self.train_reader.read(str(self.dataset_train_file_path)))[:100]
            self.test_instances = list(self.test_reader.read(str(self.dataset_test_file_path)))
    ...
    def test_model_training(self):
        ...
        val_iterator = BucketIterator(sorting_keys=[("source_tokens", "num_tokens")], padding_noise=0.0, batch_size=batch_size)
        ...
        trainer = CallbackTrainer(model=self.model,
                                  ...
                                  callbacks=[LogToTensorboard(tensorboard),
                                             Validate(validation_data=training_dataset, validation_iterator=train_iterator),
        ...
        for i in range(50):
            ...
            predictor = Seq2SeqPredictor(self.model, self.test_reader)
